...
export class PengarsipanIjazah extends Contract {
    ...
    @Transaction()
    public async CreateAsset(ctx: Context, no_ijazah: string, nisn: string, nis: string, nama: string, jk: string, nama_orangtua: string, prodi: string, arsip_ijazah: string, konfirmasi_kepsek: string, konfirmasi_kesiswaan: string): Promise<String> {
        ...
        await ctx.stub.putState(no_ijazah, Buffer.from(stringify(sortKeysRecursive(asset))));
        const idTx = ctx.stub.getTxID()
        return 'Pengarsipan data ijazah telah berhasil dilakukan, ID Transaksi: ' + idTx;
    }
    ...
    @Transaction()
    public async UpdateAsset(ctx: Context, no_ijazah: string, nisn: string, nis: string, nama: string, jk: string, nama_orangtua: string, prodi: string, arsip_ijazah: string, konfirmasi_kepsek: string, konfirmasi_kesiswaan: string): Promise<String> {
        const exists = await this.AssetExists(ctx, no_ijazah);
        if (!exists) {
            throw new Error(`The asset ${no_ijazah} does not exist`);
        }
        ...
        // we insert data in alphabetic order using 'json-stringify-deterministic' and 'sort-keys-recursive'
        await ctx.stub.putState(no_ijazah, Buffer.from(stringify(sortKeysRecursive(updatedAsset))));
        const idTrx = ctx.stub.getTxID();
        return 'Ubah data arsip ijazah telah berhasil diubah, ID Transaksi: ' + idTrx;
    }
